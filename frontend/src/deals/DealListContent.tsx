  ...
  const onDragEnd: OnDragEndResponder = async (result) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      if (source.index > destination.index) {
        ...
        await Promise.all([
          ...
          ...columnDeals
            .filter(
              (deal) =>
                deal.anindex >= destinationDeal.anindex &&
                deal.anindex < sourceDeal.anindex
            )
            .map((deal) =>
              dataProvider.update("deals", {
                id: deal.id,
                data: { anindex: deal.anindex + 1 },
                previousData: deal,
              })
            ),
          ...
          dataProvider.update("deals", {
            id: sourceDeal.id,
            data: { anindex: destinationDeal.anindex },
      ...
      await Promise.all([
        ...
        ...sourceDeals
          .filter((deal) => deal.anindex > sourceDeal.anindex)
          .map((deal) =>
            dataProvider.update("deals", {
              id: deal.id,
              data: { anindex: deal.anindex - 1 },
        ...
        ...destinationDeals
          .filter((deal) =>
            destinationDeal ? deal.anindex >= destinationDeal.anindex : false
          )
          .map((deal) =>
            dataProvider.update("deals", {
              id: deal.id,
        ...
        dataProvider.update("deals", {
          id: sourceDeal.id,
          data: {
            anindex: destinationDeal
              ? destinationDeal.anindex
              : destinationDeals.pop()!.anindex + 1,
